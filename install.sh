...
install_pi() {
	if [[ "$INSTALL_PI" -ne 1 ]]; then
		log "pi install disabled by default; use --install-pi to enable it"
		return
	fi

	command -v curl >/dev/null 2>&1 || die "curl is required. Install curl first, then rerun ./install.sh"

	log "installing/updating pi via official installer"
